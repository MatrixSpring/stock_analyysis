...
class EfinanceFetcher(BaseFetcher):
    ...
    # ============================================================
    # Phase 1-3 新增：筹码分布 + 社区舆情（efinance 主源实现）
    # ============================================================
    ...
    async def get_chip_distribution(
        ...
        def _sync_fetch():
            try:
                ...
                if not hasattr(ef.stock, "get_latest_chip_distribution"):
                    ...
                    return
                data = ef.stock.get_latest_chip_distribution(code)
        ...
        try:
            chip_data = result_queue.get_nowait()
        except Exception:
            return None

        if chip_data is None:
            return None
        if hasattr(chip_data, "empty") and chip_data.empty:
            return None
        ...
        if isinstance(chip_data, dict):
            result["avg_cost"] = float(chip_data.get("avg_cost", 0) or 0)
            result["profit_ratio"] = float(chip_data.get("profit_ratio", 0) or 0)
        elif hasattr(chip_data, "to_dict"):
            result["raw"] = chip_data.to_dict() if hasattr(chip_data.to_dict, "__call__") else str(chip_data)
    ...
    async def get_stock_sentiment(
        ...
        def _sync_fetch():
            try:
                import efinance as ef
                if not hasattr(ef.stock, "get_comments"):
                    result_queue.put([])
                    return
                df = ef.stock.get_comments(code)
                result_queue.put(df)
            except Exception:
                result_queue.put([])
        ...
        for _, row in df.head(limit).iterrows():
            ...
            results.append({
                "sentiment_id": f"ef_{code}_{row.get('评论id', row.get('id', ''))}",
                "code": code, "market": market,
