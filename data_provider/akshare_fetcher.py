...
from .realtime_types import (
    ...
    compute_simple_sentiment,  # Phase 3: 舆情情感打分
)
...
class AkshareFetcher(BaseFetcher):
    ...
    # ============================================================
    # Phase 1-3 新增：基本面 / 新闻 / 舆情备选采集（akshare 实现）
    # ============================================================
    ...
    async def get_fundamental(
        ...
        """
        获取基本面财务数据（akshare 主源）。

        获取内容：利润表 + 资产负债表 + 主要财务指标
        返回标准化 dict 列表。
        """
        ...
        def _sync_fetch():
            ...
            try:
                ...
                # 1. 获取主要财务指标（包含 ROE/毛利率/净利率/PE/PB 等）
                try:
                    indicators = ak.stock_financial_abstract_ths(symbol=code, indicator="按报告期")
                    if indicators is not None and not indicators.empty:
                        latest = indicators.head(4)  # 最近4期
                        for _, row in latest.iterrows():
                            report = {
                                "report_period": str(row.get("报告期", row.get("date", ""))).replace("-", ""),
                                "announce_date": "",
                                "pe_ttm": safe_float(row.get("市盈率", row.get("PE", None))),
                                "pb": safe_float(row.get("市净率", row.get("PB", None))),
                                "roe": safe_float(row.get("净资产收益率", row.get("ROE", None))),
                                "gross_margin": safe_float(row.get("毛利率", None)),
                                "net_margin": safe_float(row.get("净利率", None)),
                                "revenue": safe_float(row.get("营业总收入", row.get("revenue", None))),
                                "net_profit": safe_float(row.get("净利润", row.get("net_profit", None))),
                                "debt_ratio": safe_float(row.get("资产负债率", None)),
                            }
                            # 去除全 None 的报告
                            if any(v is not None for v in report.values() if v != report.get("report_period")):
                                reports.append(report)
                except Exception:
                    pass

                # 2. 备选：stock_financial_analysis_indicator
                if not reports:
                    try:
                        ...
                        if df is not None and not df.empty:
                            ...
                            for _, row in latest.iterrows():
                                report = {
                                    "report_period": str(row.get("日期", row.get("date", ""))).replace("-", ""),
                                    "roe": safe_float(row.get("净资产收益率(%)", row.get("ROE", None))),
                                    "gross_margin": safe_float(row.get("销售毛利率(%)", None)),
                                    "net_margin": safe_float(row.get("销售净利率(%)", None)),
                                    "revenue": safe_float(row.get("营业总收入(元)", row.get("revenue", None))),
                                    "net_profit": safe_float(row.get("归属母公司净利润(元)", None)),
                                }
                                if any(v is not None for v in report.values() if v != report.get("report_period")):
                                    reports.append(report)
                    except Exception:
            ...
            result_queue.put(reports)

        t = Thread(target=_sync_fetch, daemon=True)
        t.start()
        t.join(timeout=20.0)

        try:
            reports = result_queue.get_nowait()
        except Exception:
            reports = []
        ...
        for r in reports:
            results.append({
                ...
                "revenue": r.get("revenue"), "net_profit": r.get("net_profit"),
                "debt_ratio": r.get("debt_ratio"),
                "source": "akshare",
                "crawl_at": datetime.now(timezone.utc).isoformat(),
                "quality": "ok" if len(r) > 3 else "partial",
            })
        return results

    async def get_stock_news(
        self, code: str, market: str = "a",
        limit: int = 50, start_dt: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        """获取个股新闻资讯（akshare 主源，含巨潮公告 + 财经快讯）"""
        from threading import Thread
        from queue import Queue
        result_queue: Queue = Queue()
        ...
        def _sync_fetch():
            ...
            try:
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        for _, row in df.head(limit).iterrows():
                            ...
                            articles.append({
                                "news_id": f"ak_{code}_{hash(title) & 0xFFFFFFFF:08x}",
                                "code": code, "market": market,
                                "title": title,
                                "summary": str(row.get("内容", row.get("content", "")))[:500],
                                "publish_time": str(row.get("发布时间", row.get("publish_time", ""))),
                                "source_platform": str(row.get("来源", row.get("source", "eastmoney"))),
                                "news_level": "L2",
                ...
                if len(articles) < limit:
                    try:
                        df2 = ak.stock_notice_report(symbol=code)
                        if df2 is not None and not df2.empty:
                            for _, row in df2.head(limit // 2).iterrows():
                                title = str(row.get("标题", row.get("title", "")))
                                articles.append({
                                    "news_id": f"ak_juchao_{code}_{hash(title) & 0xFFFFFFFF:08x}",
                                    "code": code, "market": market,
                                    "title": title,
                                    "summary": str(row.get("摘要", row.get("summary", "")))[:500],
                                    "publish_time": str(row.get("发布时间", row.get("publish_time", ""))),
    ...
    async def get_stock_sentiment(
        self, code: str, market: str = "a",
        limit: int = 100, start_dt: Optional[str] = None,
    ) -> List[Dict[str, Any]]:
        """获取雪球/股吧社区舆情（akshare 备选）"""
        from threading import Thread
        from queue import Queue
        result_queue: Queue = Queue()
        ...
        def _sync_fetch():
            ...
            try:
                ...
                # 雪球热帖（如有接口）
                try:
                    df = ak.stock_hot_discuss_xq(symbol=code)
                    if df is not None and not df.empty:
                        for _, row in df.head(limit).iterrows():
                            content = str(row.get("text", row.get("内容", "")))
                            title = str(row.get("title", row.get("标题", content[:50])))
                            items.append({
                                "sentiment_id": f"ak_xq_{code}_{hash(content) & 0xFFFFFFFF:08x}",
                                "code": code, "market": market,
                                "title": title, "content": content,
                                "publish_time": str(row.get("created_at", row.get("发布时间", ""))),
                                "source_platform": "xueqiu",
                                "like_count": int(row.get("like_count", row.get("点赞", 0)) or 0),
                                "reply_count": int(row.get("reply_count", row.get("回复", 0)) or 0),
                                "author": str(row.get("user_name", row.get("作者", ""))),
                                "sentiment_score": compute_simple_sentiment(f"{title} {content}"),
                                "source": "akshare",
                                "crawl_at": datetime.now(timezone.utc).isoformat(),
                            })
                except Exception:
                ...
                if not items:
                    try:
                        df2 = ak.stock_zh_a_comment_em(symbol=code)
                        if df2 is not None and not df2.empty:
                            for _, row in df2.head(limit).iterrows():
                                content = str(row.get("内容", row.get("content", "")))
                                title = str(row.get("标题", row.get("title", content[:50])))
                                items.append({
                                    "sentiment_id": f"ak_em_{code}_{hash(content) & 0xFFFFFFFF:08x}",
                                    "code": code, "market": market,
                                    "title": title, "content": content,
                                    "publish_time": str(row.get("发布时间", row.get("publish_time", ""))),
                                    "source_platform": "eastmoney",
                                    "like_count": int(row.get("点赞", row.get("like_count", 0)) or 0),
                                    "reply_count": int(row.get("回复", row.get("reply_count", 0)) or 0),
                                    "author": str(row.get("作者", row.get("author", ""))),
                                    "sentiment_score": compute_simple_sentiment(f"{title} {content}"),
                                    "source": "akshare",
                                    "crawl_at": datetime.now(timezone.utc).isoformat(),
                                })
