...
class StockAnalysisPipeline:
    ...
    def analyze_stock(
        ...
        try:
            ...
            # 多模型共识增强（可选）
            if result:
                self._maybe_enhance_with_multi_model(result, code, stock_name)
    ...
    # ---- Multi-Model Consensus Enhancement ----

    @property
    def _multi_model_svc(self):
        """Lazy-loaded multi-model analysis service."""
        if getattr(self, "_cached_multi_model_svc", None) is None:
            try:
                ...
                if svc.enabled:
                    ...
                    self._cached_multi_model_svc = svc
                else:
                    self._cached_multi_model_svc = False  # sentinel for "tried, disabled"
            except Exception as e:
                logger.debug("Multi-model service init failed: %s", e)
                self._cached_multi_model_svc = False
        svc = self._cached_multi_model_svc
        return svc if svc and svc is not False else None

    def _maybe_enhance_with_multi_model(
        self, result: Any, code: str, stock_name: str,
    ) -> None:
        """Post-analysis: optionally enhance result with multi-model consensus."""
        if result is None:
            return
        svc = self._multi_model_svc
        if svc is None:
            return

        try:
            # Build minimal context for consensus analysis
            stock_context = f"股票: {stock_name}({code})"
            if hasattr(result, "price"):
                stock_context += f"\n价格: {getattr(result, 'price', 'N/A')}"
            if hasattr(result, "sentiment_score"):
                stock_context += f"\nAI评分: {getattr(result, 'sentiment_score', 'N/A')}"

            # Use the existing analysis conclusion as the prompt base
            base_conclusion = ""
            if hasattr(result, "conclusion"):
                base_conclusion = str(getattr(result, "conclusion", ""))[:200]
            if hasattr(result, "summary"):
                base_conclusion += "\n" + str(getattr(result, "summary", ""))[:200]

            prompt = (
                f"请对以下股票进行多维度分析（趋势方向、风险评估、操作策略），"
                f"要求给出具体数据支撑和概率评分:\n"
                f"主分析结论: {base_conclusion or '待分析'}"
            )

            consensus_result = svc.analyze(stock_context, prompt)
            if consensus_result and consensus_result.get("consensus"):
                # Attach to result
                setattr(result, "multi_model_consensus",
                        consensus_result["consensus"])
                setattr(result, "multi_model_stats",
                        consensus_result.get("stats", {}))
                setattr(result, "multi_model_results",
                        consensus_result.get("results", []))

                logger.info(
                    "[%s] Multi-model consensus enhanced: "
                    "reliability=%s, models=%s/%s",
                    code,
                    consensus_result["consensus"].get("reliability_score", "?"),
                    consensus_result["stats"].get("success", 0),
                    consensus_result["stats"].get("total", 0),
                )
        except Exception as e:
            logger.warning("[%s] Multi-model enhancement skipped: %s", code, e)
