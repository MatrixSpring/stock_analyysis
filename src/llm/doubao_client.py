...
class DoubaoClient(BaseLLMClient):
    def __init__(self):
        ...
        self.endpoint = settings.DOUBAO_ENDPOINT or "https://ark.cn-beijing.volces.com/api/v3"
    ...
    def chat(
        ...
        try:
            ...
            logger.error(f"豆包LLM调用失败 {str(e)}")
            raise LLMServiceError(f"豆包API请求异常：{str(e)}") from e


# 全局单例
doubao_llm = DoubaoClient()
